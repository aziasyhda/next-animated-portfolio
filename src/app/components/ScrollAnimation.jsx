...
export default function ScrollAnimation() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (scrollY > 100) {
        // Start animation when scrolled past 100px
        controls.start({ opacity: 1, y: 0 });
      } else {
        // Reset animation when above 100px
  ...
  return (
    <motion.div
      initial={{ opacity: 0, y: -50 }}  // Initial state when not animated
      animate={controls}                // Controls the animation dynamically
      transition={{ duration: 1 }}     // Duration of the animation
      className="bg-red-500 p-6 rounded text-white"
    >
      Scroll to see me animate!
    </motion.div>
  );
}*/}//scrol fade zoom out in
